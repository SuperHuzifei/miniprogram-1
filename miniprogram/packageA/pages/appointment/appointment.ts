...
Page({
  ...
  goToLogin() {
    wx.navigateTo({
      url: '/pages/login/login?redirect=' + encodeURIComponent('/packageA/pages/appointment/appointment')
    });
  },
    ...
    wx.cloud.callFunction({
      ...
      success: (res: any) => {
        ...
        if (success) {
          // 创建预约成功后，跳转到预约页面
          wx.navigateTo({
            url: `/packageA/pages/reservation/reservation?appointmentId=${data.appointmentId}&amount=${data.amount}&hours=${data.hours}`
          });
        } else {
          Message.error({
            context: this,
